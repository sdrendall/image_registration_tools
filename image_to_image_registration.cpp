...
#include "transform_application.h"
#include "image_io.h"
...
int main(int argc, char** argv) {
    ...
    moving_image = apply_transform<IMAGE_TYPE>(moving_image, rigid_transform);
    BSPLINE_TRANSFORM_TYPE::Pointer bspline_transform = compute_bSpline_transform(fixed_image, moving_image);

    // Apply tranform
    IMAGE_TYPE::Pointer output_image = apply_transform<IMAGE_TYPE>(moving_image, bspline_transform);
    ...
    if (options[TRANSFORM_PATH]) {
       ...
       write_transform(composite_transform, options[TRANSFORM_PATH].arg);
    }
